...
from ..task_states import WorkflowState, TaskTree, Milestone, MilestoneStatus
...
class DashboardRenderer(UIBase):
    """Dashboard renderer that implements UIBase and subscribes to MessageHistory.
    
    Maintains compatibility with existing code while adding MessageHistory subscription.
    """
    ...
    def __init__(
        ...
        status_history: Optional[StatusHistory] = None,
        ...
        Args:
            ...
            message_history: Optional MessageHistory to subscribe to
        ...
        # Subscribe to message history if provided
        if self.message_history:
            self.message_history.subscribe(self.on_message)
            self.message_history.subscribe_completion(self.on_message_complete)
        
        # Subscribe to status history if provided
        if self.status_history:
            self.status_history.subscribe(self.on_status_event)
        
        # Subscribe to UI state manager if provided
        if self.ui_state_manager:
            self.ui_state_manager.subscribe(self.on_ui_state_update)
    ...
    def on_message(self, message: Message) -> None:
        ...
        """
        # For now, we can track messages but not display them in the dashboard
        # This can be extended later to show message history in detailed view
        # The dashboard primarily focuses on node/graph status, not message content
        pass
    
    def on_message_complete(self, message: Message) -> None:
        """Handle message completion.
        
        Args:
            message: Completed message
        """
        # Message completed - can be used for summaries or logging
        pass
    ...
    def on_status_event(self, event: StatusEvent) -> None:
        ...
        """
        # Update dashboard based on status event
        if event.type == StatusEventType.NODE_START:
            # Node started - update dashboard state
            # The app.update_state will be called from graph tracking, but we can
            # also update based on status events for consistency
            pass
        
        elif event.type == StatusEventType.NODE_COMPLETE:
            # Node completed - ensure dashboard reflects this
            if event.node_name:
                # Update node status in dashboard
                self.app.dashboard_state.node_statuses[event.node_name] = NodeStatus.COMPLETE
                # Add to recent updates
                self.app.dashboard_state.last_updates.append(event.summary)
                self.app._update_widgets()
        
        elif event.type == StatusEventType.NODE_FAILED:
            # Node failed
            if event.node_name:
                self.app.dashboard_state.node_statuses[event.node_name] = NodeStatus.FAILED
                self.app.dashboard_state.last_updates.append(f"Failed: {event.summary}")
                self.app._update_widgets()
        
        elif event.type == StatusEventType.TASK_COMPLETE:
            # Task completed
            self.app.dashboard_state.last_updates.append(event.summary)
            self.app._update_widgets()
        
        elif event.type == StatusEventType.TASK_FAILED:
            # Task failed
            self.app.dashboard_state.last_updates.append(event.summary)
            self.app._update_widgets()
        
        elif event.type == StatusEventType.MILESTONE_ADVANCE:
            # Milestone advanced
            self.app.dashboard_state.last_updates.append(event.summary)
            self.app._update_widgets()
        
        elif event.type == StatusEventType.ITERATION_INCREMENT:
            # Iteration incremented
            self.app.dashboard_state.last_updates.append(event.summary)
            self.app._update_widgets()
    
    def on_ui_state_update(self, ui_state: UIState) -> None:
        """Handle UI state updates from UIStateManager.
        
        This is called when UIState changes (throttled updates).
        Dashboard can use UIState to update its widgets with the latest state.
        
        Args:
            ui_state: Updated UIState
        """
        # Update dashboard state from UIState
        if ui_state.current_node:
            self.app.dashboard_state.current_node = ui_state.current_node
            # Update node statuses
            for node_name, status in ui_state.node_statuses.items():
                if isinstance(status, str):
                    try:
                        self.app.dashboard_state.node_statuses[node_name] = NodeStatus(status)
                    except ValueError:
                        # If status string doesn't match NodeStatus enum, use PENDING
                        self.app.dashboard_state.node_statuses[node_name] = NodeStatus.PENDING
        
        # Update app's internal state with UIState data
        # We update the app's state dict directly to keep widgets in sync
        if hasattr(self.app, 'state'):
            # Update relevant fields from UIState
            self.app.state["current_node"] = ui_state.current_node
            self.app.state["node_statuses"] = {k: v if isinstance(v, str) else str(v) 
                                                for k, v in ui_state.node_statuses.items()}
    ...
    def update_state(self, state: WorkflowState, node_name: Optional[str] = None):
        ...
        This handles node/graph status updates (separate from message history).
    ...
    def cleanup(self):
        ...
        # Unsubscribe from status history
        if self.status_history:
            self.status_history.unsubscribe(self.on_status_event)
        # Unsubscribe from UI state manager
        if self.ui_state_manager:
